...
def linearReg2(df, features):
    print('\n===Linear Reg 2===')
    feature_copy = features.copy()
    del feature_copy[:3]
    del feature_copy[3]
    y_COL = 'gold_earned'
    X_train = df[feature_copy]
    y_train = df[y_COL]
    X_test = df[feature_copy]
    y_test = df[y_COL]
    ...
    plt.title('Linear Regression 2 (Predict Total)')
    plt.xlabel('Actual Value')
    plt.ylabel('Predicted Value')
    plt.savefig('linearReg2.png')
    plt.close()
    ...
    plt.xlabel('Fitted')
    plt.ylabel('Residual')
    plt.savefig('Residual_Graph2.png')
    plt.close()
